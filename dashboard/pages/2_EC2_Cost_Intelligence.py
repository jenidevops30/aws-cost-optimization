...
try:
    with st.spinner("Reading EC2 cost, inventory and CloudWatch metrics…"):
        ...
        rows = [{"resource_id": r.resource_id, "cost": r.cost, "billing_period": r.billing_period} for r in costs if r.resource_id]
        ...
        metric_rows = get_ec2_utilization(instance_ids, datetime.combine(metric_start, time.min, tzinfo=timezone.utc), datetime.combine(end, time.min, tzinfo=timezone.utc), region=region, period=period)
...
st.dataframe(frame.style.format({"cost": "${:,.2f}", "cost_share_pct": "{:.1f}%", "cpu_avg_pct": "{:.1f}%", "cpu_max_pct": "{:.1f}%"}), use_container_width=True, hide_index=True)
...
st.subheader("Cost review signals")
for signal in ["arm64-review", "stopped-resource-review", "high-cost-concentration"]:
    count = int(frame["signals"].str.contains(signal, regex=False).sum())
    st.write(f"**{signal}** — {count} resource(s)")

st.warning("Signals are review candidates, not automatic rightsizing decisions. CPU utilization is evidence for investigation, not a complete capacity model; memory utilization requires a separately installed CloudWatch agent/custom metric and is not inferred here.")
