    ...
    while True:
        ...
        for anomaly in response.get("Anomalies", []):
            ...
            result.append({
                ...
                "max_impact_usd": float(impact.get("TotalActualSpend", 0) or 0),
                "root_causes": [
                    {
...
def summarize_anomalies(anomalies: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    services = sorted({
        str(cause.get("service"))
        for item in anomalies
        for cause in item.get("root_causes", [])
        if cause.get("service")
    })
    return {
        "anomaly_count": len(anomalies),
        "total_estimated_impact_usd": round(sum(impacts), 2),
        "max_estimated_impact_usd": round(max(impacts), 2) if impacts else 0.0,
