...
def _query(instance_id: str, metric_name: str, period: int, stat: str) -> dict[str, Any]:
    return {
...
def get_ec2_utilization(
    ...
    *,
    ...
    CPU is summarized from Average and Maximum datapoints. NetworkIn and
    NetworkOut are summed as bytes over the requested period. Missing metrics
    remain None rather than being interpreted as zero utilization.
    """
    if end <= start:
    ...
    for instance_id in instance_ids:
        queries = [
            {**_query(instance_id, "CPUUtilization", period, "Average"), "Id": "cpuavg"},
            {**_query(instance_id, "CPUUtilization", period, "Maximum"), "Id": "cpumax"},
            {**_query(instance_id, "NetworkIn", period, "Sum"), "Id": "networkin"},
            {**_query(instance_id, "NetworkOut", period, "Sum"), "Id": "networkout"},
        ]
        ...
        network_in = values("networkin")
        network_out = values("networkout")
        results.append(
            {
