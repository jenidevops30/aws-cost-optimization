...
def correlate_cost_and_utilization(
    ...
    *,
    ...
    for row in cost_rows:
        ...
        output.append({
            **row,
            "cpu_average_pct": cpu,
            "cpu_max_pct": metrics.get("cpu_max_pct"),
            "network_in_bytes": metrics.get("network_in_bytes"),
            "network_out_bytes": metrics.get("network_out_bytes"),
            "cpu_datapoints": metrics.get("cpu_datapoints", 0),
            "utilization_status": "available" if metrics.get("cpu_datapoints", 0) else "no-cpu-data",
