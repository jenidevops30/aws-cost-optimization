...
def _query(volume_id: str, metric_name: str, period: int, stat: str, query_id: str) -> dict[str, Any]:
    return {"Id": query_id, "MetricStat": {"Metric": {"Namespace": "AWS/EBS", "MetricName": metric_name, "Dimensions": [{"Name": "VolumeId", "Value": volume_id}]}, "Period": period, "Stat": stat}, "ReturnData": True}


def get_ebs_metrics(volume_ids: list[str], start: datetime, end: datetime, *, region: str, period: int = 300, client=None) -> list[dict[str, Any]]:
    ...
    metrics = [
        ("VolumeReadOps", "Average", "read_ops"), ("VolumeWriteOps", "Average", "write_ops"),
        ("VolumeReadBytes", "Average", "read_bytes"), ("VolumeWriteBytes", "Average", "write_bytes"),
        ("VolumeQueueLength", "Average", "queue"), ("VolumeIdleTime", "Average", "idle_time"),
    ]
    ...
    for volume_id in volume_ids:
        ...
        results.append({
            ...
            "read_ops_datapoints": len(rows["read_ops"]), "write_ops_datapoints": len(rows["write_ops"]),
            "mode": "analysis-only",
        })
    return results
