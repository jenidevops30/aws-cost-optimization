...
def scan_repository(root: str | Path) -> list[ComplianceCheck]:
    ...
    findings = 0
    scanned = 0
    allowed = {".py", ".yml", ".yaml", ".json", ".toml", ".ini", ".cfg", ".env", ".md", ".txt"}
    ...
    checks = [ComplianceCheck("secret-pattern-scan", "ready" if findings == 0 else "failed", f"scanned={scanned}; findings={findings}")]
